...
class ApiService {
    static async request(url, options = {}) {
        try {
            ...
            // This error is often due to Mixed Content blocking (calling http from https)
...
class FixerService {
    ...
    // CRITICAL FIX: The free plan requires 'http'.
    // NOTE: If the app is hosted on HTTPS, browsers will block this 'http' call (Mixed Content).
    // This is a limitation of the free Fixer.io API, not the code itself.
    static get BASE_URL() { return 'http://data.fixer.io/api/latest'; }
    static get SUPPORTED_CURRENCIES() { return ['USD', 'EUR', 'GBP', 'CAD', 'AUD', 'ZAR', 'BWP', 'ZWG', 'CNY', 'JPY', 'KES', 'NGN', 'INR', 'CHF', 'NZD']; }

    static async getExchangeRates(forceRefresh = false) {
        const cachedRatesData = DataStore.get('exchangeRates');
        const oneHour = 3600000;

        if (!forceRefresh && cachedRatesData && (Date.now() - cachedRatesData.timestamp < oneHour)) {
            console.log(`Using ${cachedRatesData.source} exchange rates from cache.`);
            return { rates: cachedRatesData.rates, source: cachedRatesData.source };
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                if (!eurToUsdRate) throw new Error('USD rate not found in Fixer.io response.');
                
                const usdBasedRates = {};
                for (const currency in eurRates) {
                    usdBasedRates[currency] = eurRates[currency] / eurToUsdRate;
                }
                usdBasedRates['USD'] = 1.0;
                
                DataStore.set('exchangeRates', { rates: usdBasedRates, timestamp: Date.now(), source: 'api' });
                return { rates: usdBasedRates, source: 'api' };
            } else {
                throw new Error(data.error?.info || 'Fixer.io API request was not successful.');
            }
        } catch (error) {
            console.error("Fixer.io API failed, likely due to browser blocking or network issues. Using fallback rates.", error.message);
            const fallbackRates = {
...
class CurrencyManager {
    ...
    async refreshRates() {
        console.log("Forcing refresh of exchange rates from Fixer.io...");
        const ratesData = await FixerService.getExchangeRates(true);
        this.rates = ratesData.rates;
        this.source = ratesData.source;
        console.log(`Rates refreshed. Source: ${this.source}`);
    }
...
class UIManager {
    ...
    setupEventListeners() {
        ...
        document.body.addEventListener('submit', e => {
            if (e.target.id === 'addTransactionForm') this.handleAddTransaction(e);
            if (e.target.id === 'addBudgetForm') this.handleAddBudget(e);
            if (e.target.id === 'addGoalForm') this.handleAddGoal(e);
        });
        ...
        document.getElementById('refreshRatesBtn')?.addEventListener('click', async () => {
            ...
            await this.app.currencyManager.refreshRates();
            this.updateUI();
    ...
    renderCurrencyRates() {
        ...
        const existingWarning = header?.querySelector('.fallback-warning');
        ...
        container.innerHTML = currenciesToShow.map(currency => `
            <div class="flex justify-between items-center mb-2">
                ...
                <span class="text-right font-medium">${rates[currency].toFixed(4)}</span>
            </div>
        `).join('');
    }
    ...
    // FIX: Updated to create a responsive, side-by-side layout for the overview cards.
    getDashboardHTML() {
        return `
            <div id="dashboardView">
                ...
                <div class="dashboard-grid">
                    ...
                    <!-- Wrapper for side-by-side overview cards -->
                    <div style="grid-column: 1 / -1; display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 1.5rem;">
                        <div class="card">
                            <div class="card-header"><h3 class="card-title">Market Overview (FMP)</h3></div>
                            <div id="marketOverview"></div>
                        </div>
                        <div class="card">
                            <div class="card-header" style="display: flex; align-items: center;"><h3 class="card-title">Exchange Rates (Fixer.io)</h3></div>
...
export class App {
    constructor() {
        ...
        // The init call is now handled by main.js after the DOM is ready
    }
